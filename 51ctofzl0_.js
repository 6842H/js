...
function heart_beat(uid, uuid, lessonId, dt, interval=20, step=60, n=0){
    var ht=n*step;
    ...
    var ext={
        ...
        pt: ht,
        t: T,
        sgin: fake("" + T + ht + dt + ht + "eDu_51Cto_siyuanTlw").toString()
    }
    try{
        $.get(url_stat, $.extend({}, i, ext));
        $.ajax({  
            async : 1,  
            type : 'get',  
            url : url_stat,  
            data : $.extend({}, i, ext),
            success: function(e){window.steps -=1;console.debug('window.steps=', window.steps);},
            error:function(e){window.steps -=1;console.debug('window.steps=', window.steps);}
        });  
    }catch(err){
        console.debug('ERR:', err.message);
    }
    console.debug('ht='+ht+', T='+T);
    if(ht<(dt+m_step*step)){setTimeout(function(){ heart_beat(uid, uuid, lessonId, dt, interval, step, n+1);}, interval);}
    else{window.clc=window.clc-1;}
}
...
function widt(interval=5, step=150){
    var lis=$("li.lesson");
    
    if(lis.length<1){alert('获取课程列表为空');return;};
    var items = [];
    ...
    if(items.length<1){alert('获取未完成课程列表为空2');return;};
    window.clc=items.length;
    
    var uid=get_uid(items[0][0]);
    window.steps = 0;
    for(var i=0;i<items.length;i++){
        window.steps += Math.floor(items[i][2]/step)+m_step-1;
    }
    for(var i=0;i<items.length;i++){
        var uuid=fake("" + (new Date).getTime() + uid + Math.random()).toString();
        console.debug('lessonId=',items[i][0], 'time=', items[i][2]);
        heart_beat(uid, uuid, items[i][0], items[i][2], interval, step);
    }
    check_clc();
    check_steps();
}
